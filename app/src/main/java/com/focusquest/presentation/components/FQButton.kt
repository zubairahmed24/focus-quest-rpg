 ...
 * for "START FOCUSING"/victory), Secondary (neutral surface), Outline.
 ...
 */
enum class FQButtonVariant { Primary, Cta, Secondary, Outline }
...
fun FQButton(
    ...
    when (variant) {
        FQButtonVariant.Outline -> OutlinedButton(
            onClick = onClick,
            modifier = modifier,
            enabled = enabled,
            shape = shape,
            border = BorderStroke(1.5.dp, if (enabled) c.brand else c.border),
            ...
            val (container, on) = when (variant) {
                ...
                FQButtonVariant.Outline -> Color.Unspecified to Color.Unspecified
            }
            Button(
                onClick = onClick,
                modifier = modifier,
